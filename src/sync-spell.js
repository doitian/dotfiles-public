...
const WIN_DIC = join(home(), "AppData", "Roaming", "Microsoft", "Spelling", "neutral", "default.dic");
const OBSIDIAN_HARPER = join(home(), "Dropbox", "Brain", ".obsidian", "plugins", "harper", "data.json");
...
async function main() {
  ...
  if (isWin && (await exists(WIN_DIC))) {
    ...
    const words = content.replace(/\uFEFF/, "").split(/\r?\n/).map((s) => s.trim()).filter(Boolean);
  ...
  const sorted = [...merged].sort((a, b) => a.localeCompare(b, undefined, { sensitivity: "variant" }));
  ...
  if (isWin && (await exists(join(WIN_DIC, "..")))) {
    ...
    const bom = "\uFEFF";
    await writeFile(WIN_DIC, bom + sorted.join("\r\n") + (sorted.length ? "\r\n" : ""), "utf16le");
    console.log("Synchronized", WIN_DIC);
  }

  if (obsidianLoaded && obsidianData) {
    obsidianData.userDictionary = sorted;
    await writeFile(OBSIDIAN_HARPER, JSON.stringify(obsidianData, null, 2) + "\n", "utf8");
