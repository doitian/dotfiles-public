...
async function substituteVariables(content, varsDict, interactive, ask) {
  ...
  variables.sort((a, b) => (typePriority[a[1]] ?? 99) - (typePriority[b[1]] ?? 99));

  for (const [fullMatch, varType, defaultVal, choices] of variables) {
    if (!content.includes(fullMatch)) continue;
    let value = null;

    if (varType === "tabstop" || varType === "default" || varType === "choice") {
      const numMatch = fullMatch.match(/\d+/);
    ...
    if (value == null && choices != null) {
      if (interactive) {
        ...
        try {
          ...
          value = idx >= 0 && idx < choices.length ? choices[idx] : choices[0] ?? "";
        } catch {
          value = choices[0] ?? "";
        }
      } else {
...
async function printPrompt(name, registry, varsDict, interactive, ask) {
  ...
  const substituted = await substituteVariables(content, varsDict, interactive, ask);
...
async function choosePrompt(registry, varsDict, interactive, ask) {
  ...
  if (selected) await printPrompt(selected, registry, varsDict, interactive, ask);
}
...
function parseArgs(argv) {
  ...
  const result = { command: "choose", var: [], input: undefined, noInteractive: false };
  let i = 0;
  ...
  while (i < args.length) {
    if (args[i] === "--var" && args[i + 1]) {
      ...
      i += 1;
    } else if (result.command === "print" && args[i] && !args[i].startsWith("-")) {
      result.name = args[i];
      i += 1;
    } else {
      i += 1;
    ...
    if (args.command === "list") {
      ...
      try {
        ...
        if (e?.code === "ENOENT" || e?.errno === -2) {
          console.error("Error: fzf not found. Please install fzf to use the choose command.");
          process.exit(1);
        }
        throw e;
      }
