...
basic_auth = base64.b64encode(f":{os.environ['GITHUB_TOKEN']}".encode()).decode()
...
if len(sys.argv) > 1:
    ...
    tag_rev = _str(
        subprocess.check_output(["git", "rev-list", "--tags", "--max-count=1"]).strip()
    )
    since = _str(
        subprocess.check_output(["git", "describe", "--tags", tag_rev]).strip()
    )
...
pr_numbers = []
if "EXTRA_PRS" in os.environ:
    pr_numbers = os.environ["EXTRA_PRS"].split(",")
...
for pr_number in pr_numbers:
    ...
    if module:
        title = "-   #{0} **{1}:** {2} (@{3})".format(pr_number, module, message, user)
    else:
    ...
    if is_breaking:
        breaking_banner = ", ".join(label for label in labels if label.startswith("b:"))
    ...
    for scope, extra_changes in extra.items():
        ...
        for change in extra_changes:
            changes[scope].append(
                Change(
                    scope, change.get("module"), change["title"], change.get("text", "")
                )
            )
