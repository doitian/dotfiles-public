                    ...
                    for used_words in range(3, len(title_words)):
                        ...
                        if last_word[0].isupper():
                            ...
                            if (
                                ...
                                and new_citation not in conflicts
                            ):
                                citations[item["id"]] = new_citation
                                conflicts[new_citation].append(item)
                                fixed = True
    ...
    payload = json.dumps(
        {
            ...
            "params": [[citekey], {"id": csl, "contentType": "text"}],
            "id": 1,
        }
    ).encode("utf-8")
    request = urllib.request.Request(
...
def build_references(citekeys, csl):
    return "\n".join(
        map(lambda key: f"[^{key}]: {build_bibliography(key, csl)}", citekeys)
    )
...
def parse_args(args=None):
    ...
    parser.add_argument(
        "--csl",
        metavar="ID",
        choices=["apa", "modern-language-association"],
        default="apa",
    )
