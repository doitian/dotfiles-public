...
name: tty7
description: >-
  Drive the tty7 terminal workbench from the shell with the `tty7` binary — list workspaces/tabs/panes, split a pane, send text or keys, capture a pane's screen, run a command in a real PTY, wait until a pane finishes or needs input, see which coding agents and ports are live. Use this whenever tty7, panes, workspaces, or `%42`/`@7`/"the other pane"/"the other agent" come up; whenever you hand work to another agent ("get Claude/Codex to do X", "派个活", "let another agent handle this", several agents in parallel); whenever something long-running or interactive (dev server, REPL, ssh, `tail -f`, a TUI) should not block your Bash tool; whenever a program needs a real terminal; and whenever you look at some *other* terminal on this machine. If `$TTY7_PANE` is set you are already inside tty7.
---
